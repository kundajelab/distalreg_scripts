...
def main():
    ...
    for name in clusters[0]:
        print("Cluster", name)
        try:
            dat = pd.read_csv("/mnt/lab_data3/kmualim/CorrelationPlotsCellLines/splitK562_files/non_expressed/files_v2/{}".format(name), sep="\t",compression="gzip")
            ...
            data_1 = data[data['num_51'].notna()]
            index = data_1[data_1['TargetGene'] == 'TargetGene'].index
            ...
            for targetgene in unique_data:
                print(targetgene)
                subset_data = data_1.loc[data_1['TargetGene']==targetgene]
                pearsonr,spearmanr, abc_scores, distance = makeDataFrame(subset_data, cells[0], name)
    ...
    celltypes = pd.read_csv("SeventyOne_CellTypes.txt", sep="\t", header=None)
...
def grabValuesFromColumns(data, column, cells):
    ...
    k562_data = data[data[column].notnull()]
    val = np.log(np.float(k562_data[column].values[0])+0.001)
    values.append(val)
     
    for i in cells:
        matched = data.loc[data['num_51']==i]
        if len(matched) != 0:
            val = np.log(np.float(matched[total_col].values[0])+0.001)
        else:
            val = 0.0
        values.append(val)
    return values
